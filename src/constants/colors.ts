export const WHITE = '#FFFFFF';

export const LIGHT_WHITE = '#FFFFFFd6';

export const GHOST_WHITE = '#F5F7FD';

export const LIGTH_GHOST_WHITE = '#E9EDFB';

export const WHISPER = '#E5E5E5';

export const VERY_LIGHT_GREY = '#cecdcd';

export const VERY_LIGHT_BLUE = '#fafaff';

export const BLACK = '#000000';

export const ZAMBEZI = '#585858';

export const PINK_SWAN = '#B3B3B3';

export const NAVY_BLUE = '#085DD7';

export const CORNFLOWER_BLUE = '#4D93F8';

export const LIGTH_BLUE = '#367EE7';

export const LIGTH_PINK = '#FCF4FD';

export const GREY = '#503939';

export const NOBEL = '#a2a2a2';

export const MANDY = '#FF000000';

export const SNUFF = '#0000FF00';

export const RED = '#FF0000FF';

export const LIGTH_RED = '#FF000063';

export const WARNING = '#FF880073';

export const DODGE_BLUE = 'dodgerblue';

export const DARK_GREEN = '#188038';

export const LIGHT_GREEN = '#BFE8CA';

export const BLUE = '#4579EE';

export const DRAWER_CONT = '#f4f6fd';

export const DRAWER_BORDER = '#e7e7e7';

export const ACTIVE = '#4D84E7';

export const ACTIVE_BACKGROUND = '#E2EDFD';

export const NOT_ACTIVE = '#828bae';

export const NOT_ACTIVE_HOVER = '#79a8ff';

export const NOT_ACTIVE_BACKGROUND_HOVER = '#e2edfd';

export const BORDER = '#d8dbe8';

export const NOTE_DOCTOR = '#4579EE';

export const CARLO_BLUE = '#4C92FF';

export const HINT = '#808080';

export const GRAY_SHADOW = '#66666666';

export const LIGHT_GRAY = '#808080';

export const UPLOAD_BUTTON_COLOR = '#3498db';

export const CONTENT_LOADER_BACKGROUND = '#f3f3f3';

export const CONTENT_LOADER_FOREGROUND = '#ecebeb';

export const POP_UP_CONTENT_BORDER = '#e6e6e6';

export const POP_UP_CONTENT_BACKGROUND = '#f8f8f8';

export const DARK_BLUE = '#060d33';

export const LIGHT_BLUE = '#081972';

export const APPOINTMENT_TIME = '#060A32';

export const ANOTHER_FUCKING_BLUE = '#4C92FF';

export const BLUEBERRY_BLUE = '#4579EE';

export const BLACK_TRANSPORT = 'rgba(0, 0, 0, 0.54)';

export const BLACK_ARROW_HOVER = 'rgba(0, 0, 0, 0.04)';

export const CALENDAR_BOX_SHADOW = 'rgba(53, 53, 53, 0.12)';

export const CALENDAR_HOVER_SHADOW = 'rgba(53, 53, 53, 0.16)';

export const PERIWINKLE = '#cbd8ff';

export const GREEN_BORDER = '#00a344';

export const NOTIFICATION_BTN_BACKGROUND = '#e6f6ed';

export const BLACK_BUT_NOT = '#00000085';

export const SOME_COLOR = '#e9380d';

export const COLOR_ONE = '#e2edfd';

export const COLOR_TWO = '#90b457';

export const COLOR_THREE = '#d7ddf4';

export const AZURE = 'azure';

export const ERROR_LIGHT = '#FEEAE8';

export const ERROR_RED = '#E9380D';
